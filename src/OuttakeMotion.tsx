		...
		>
			...
			>
				<svg width={w} height={w} viewBox={`0 0 ${w} ${w}`} style={{overflow: "visible"}}>
					...
					{/* Clapper arm group — pivots from bottom-left corner */}
					<g
						style={{
					...
					>
						...
						{/* Diagonal stripes clipped to arm shape */}
...
export const OuttakeMotion: React.FC<OuttakeMotionProps> = ({
	...
	return (
		<AbsoluteFill style={{backgroundColor: "#000"}}>
			{/* Video layer — always plays (audio keeps running) */}
			<AbsoluteFill>
				<OffthreadVideo
			...
			{/* Solid blue when wave fully covers */}
			{blueSolid && (
				<AbsoluteFill style={{backgroundColor: BG_COLOR}} />
			)}

			{/* Wave in — liquid rising from bottom */}
			{isInTransitionIn && (
				<LiquidWave progress={waveIn} color={BG_COLOR} direction="in" />
			)}

			{/* Wave out — liquid sliding out upward */}
			{isInTransitionOut && (
				<LiquidWave progress={waveOut} color={BG_COLOR} direction="out" />
			)}

			{/* Clapperboard overlay — appears from "short-form" through "clip" */}
			{showBlue && (() => {
			...
			{/* Text layer — word-by-word motion graphics during animation range */}
			{showBlue &&
